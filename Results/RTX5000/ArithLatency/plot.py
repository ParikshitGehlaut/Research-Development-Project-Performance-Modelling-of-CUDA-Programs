...
def create_plot():
    ...
    # ------------------------------
    # Fixed figsize for ALL GPUs — ensures identical physical cell size in paper.
    # RTX has 7 shmem cols, A100/H100 have 10; same fig size → RTX cells are wider,
    # which is fine and prevents numbers cramming together.
    ...
    ax = sns.heatmap(
        ...
        linecolor="gray",
        square=False,
        annot_kws={"size": 32, "weight": "bold"},   # larger now colorbar space is freed
        ax=ax,
        cbar=False,              # remove colorbar entirely — frees ~15% width for fonts
    )
    ...
    # Save formatted plot
    plot_path = os.path.join(OUTPUT_DIR, f'alu_latency_heatmap_rtx5000.png')
