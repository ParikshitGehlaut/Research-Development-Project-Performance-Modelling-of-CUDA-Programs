...
def analyze_performance(filename):
    ...
    values for runs achieving > 95% of that peak.
    ...
    # Calculate the 95% performance threshold
    performance_threshold = max_gflops * 0.85
    ...
    print(f"95% Performance Threshold: {performance_threshold:.4f} GFLOPS")
    print("-" * 30)

    if not high_perf_occupancies:
        print("No experiments were found that exceeded the 95% performance threshold.")
    else:
        print("Found Occupancy values for experiments > 95% peak performance:")
