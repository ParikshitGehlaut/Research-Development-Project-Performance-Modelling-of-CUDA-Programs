/*
    Workload : Arithmetic Latency Measurement (Volkov's Methodology)
    Author   : Parikshit Gehlaut
    Access   : Register-to-Register (No Global/Shared Memory)
    File     : alu_lat_kernel.cu

    Measures FADD, FMUL, and FMA latency via dependent instruction chains.
    No memory loads — unaffected by the 4-byte/8-byte load decision.
*/
